...
import com.github.kr328.kaidl.builder.addDescriptor
import com.github.kr328.kaidl.builder.addReadFromParcel
import com.github.kr328.kaidl.builder.addTransactProperty
import com.github.kr328.kaidl.builder.addWriteToParcel
import com.github.kr328.kaidl.builder.delegate
import com.github.kr328.kaidl.builder.descriptorProperty
import com.github.kr328.kaidl.builder.proxy
import com.github.kr328.kaidl.builder.transactionProperty
...
fun FileSpec.Builder.addProxyClass(
  ...
  val clazz =
    TypeSpec.classBuilder(forClass.proxy)
      .addSuperinterface(forClass)
      .primaryConstructor(
        FunSpec.constructorBuilder()
          .addParameter("remote", com.github.kr328.kaidl.resolver.IBINDER)
          .build()
      )
      .addProperty(
        PropertySpec.builder("remote", com.github.kr328.kaidl.resolver.IBINDER)
          .initializer("remote")
          .build()
      )

  for (f in functions) clazz.addProxy(forClass, f)
...
fun FileSpec.Builder.addUnwrap(forClass: ClassName): FileSpec.Builder {
  ...
  return addFunction(
    FunSpec.builder("unwrap")
      .addParameter("c", KClass::class.asClassName().parameterizedBy(forClass))
...
fun TypeSpec.Builder.addGetDescriptor(): TypeSpec.Builder {
  val func =
    FunSpec.builder("getInterfaceDescriptor")
...
fun TypeSpec.Builder.addOnTransact(functions: Sequence<FunSpec>): TypeSpec.Builder {
  val code =
...
fun Sequence<FunSpec>.mapOfCodes(): Map<String, Int> {
  ...
  val generateCode: () -> Int = {
    ...
    while (generatedCode in definedCodes) generatedCode++
