...
fun Random.nextSparseBooleanArray(size: Int): SparseBooleanArray {
  return SparseBooleanArray(size).apply { repeat(size) { append(it, nextBoolean()) } }
}
...
fun Random.nextSerializable(): ExampleSerializable {
  return ExampleSerializable(nextString())
}
